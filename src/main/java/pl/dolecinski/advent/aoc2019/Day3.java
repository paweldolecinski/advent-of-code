...
//            minDistance = dist;
    ...
    private static void traverse(int pathNo, String[] pathMoves, HashMap<Point, Point> visitedPoints, HashSet<Point> intersectionPoints) {
            ...
            switch (direction) {
                case 'R':
                    ...
                    for (int i = startX + 1; i <= endX; i++) {
                        Point point = new Point(i, startY, pathNo);
                        checkIntersection(intersectionPoints, visitedPoints, point, ++steps);
                    }
                    break;
                case 'L':
                    endX = startX - distance;
                ...
                case 'U':
                    ...
                    for (int i = startY + 1; i <= endY; i++) {
                        Point point = new Point(startX, i, pathNo);
                        checkIntersection(intersectionPoints, visitedPoints, point, ++steps);
                    }
                    break;
                case 'D':
    ...
    private static void checkIntersection(HashSet<Point> intersectionPoints, HashMap<Point, Point> visitedPoints, Point point, int steps) {

        if (visitedPoints.containsKey(point) && visitedPoints.get(point).pathNo != point.pathNo) {
            visitedPoints.get(point).steps2 = steps;
            intersectionPoints.add(visitedPoints.get(point));
        } else {
            point.steps1 = steps;
            visitedPoints.put(point, point);
        }
    }
    ...
    static class Point {
        ...
        public Point(int x, int y, int pathNo) {
            this.x = x;
            this.y = y;
            this.pathNo = pathNo;
        }
